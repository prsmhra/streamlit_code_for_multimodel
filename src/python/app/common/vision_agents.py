...
class MetaIntentAgent(BaseAgent):
    ...
    def __init__(self, llm_agent: LlmAgent, name="MetaIntentTool"):
    ...
    async def _run_async_impl(self, ctx):
        ...
        if not user_msg:
            yield Event(
                content=types.Content(
                    role="assistant",
                    parts=[types.Part(text="⚠️ No user message found for MetaIntent analysis.")]
                ),
                author=self.name,
            )
            return
        ...
        raw = ctx.session.state.get("meta_intent_result", "")
        
        if not raw:
            yield Event(
                content=types.Content(
                    role="assistant",
                    parts=[types.Part(text="⚠️ No meta-intent result returned by LLM.")]
                ),
                author=self.name,
            )
            return
        ...
        try:
            ...
            result = {"intent_type": "invalid_input", "disease_focus": "", "reason": "Could not parse LLM output."}
...
class CSVFilterAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx):
        ...
        if not csv_path or not os.path.exists(csv_path):
            ...
            yield Event(content=types.Content(role="assistant", parts=[types.Part(text=msg)]), author=self.name)
            return
        ...
        #     print("Content:", getattr(ev, "content", None))
        #     print("Function call info:", getattr(ev, "function_call", None))
        #     print("===================")
...
class SymptomAnalyzerAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx):
        ...
        if not csv_path or not os.path.exists(csv_path):
            ...
            yield Event(content=types.Content(role="assistant", parts=[types.Part(text=msg)]),
                        author=self.name)
            return
        ...
        disease_focus = ctx.session.state.get("disease_focus", "")
        if disease_focus:
        ...
        raw_output = ctx.session.state.get("symptom_analysis", "")
        ...
        # msg = "✅ Symptom analysis process completed."
        # yield Event(content=types.Content(role="assistant", parts=[types.Part(text=msg)]),
        #             author=self.name)
...
class LlmOrchestratorAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx):
        yield Event(
            content=types.Content(role="assistant", parts=[types.Part(text="🧭 Orchestrator is starting...")]),
            author=self.name
        )
        history = []
        ...
        intent_raw = ctx.session.state.get("meta_intent_result", "")
        try:
        ...
        while True:
            ...
            decision_raw = ctx.session.state.get("orchestrator_decision", "")
            try:
